...
function carregarDados() {
    ...
    try {
        ...
        document.getElementById('comprimento').value = dados.comprimento || '';// Criar tubos salvos
        dados.tubos.forEach((tubo, index) => {
            if (index > 0) adicionarTubo();
            const tubos = document.querySelectorAll('.tubo-item');
            if (tubos[index]) {
                tubos[index].querySelector('.raio').value = tubo.raio || '';
                tubos[index].querySelector('.quantidade').value = tubo.quantidade || '';
                tubos[index].querySelector('.comprimento-tubo').value = tubo.comprimento || '';
            }
        });
    } catch (error) {
